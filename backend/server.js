...
//Prueba de local compu Sabrina

// server.js VERSION COMPU SABRI
import express from "express";
import axios from "axios";
import dotenv from "dotenv";
import cors from "cors";
...
app.post("/login", authLimiter, async (req, res) => {
    try {
        ...
        const destination = user.admin ? "/admin/admin.html" : "/usuario/usuario.html";
        return res.json({ success: true, redirect: destination });
...
// --- AUDITORÍA (ESQUEMA CORE) ---
app.get('/api/core/audit/:instruccion', async (req, res) => {
    const { instruccion } = req.params;
    const tablasCore = ['registrar_proyecto', 'actualizar_proyecto', 'eliminar_proyecto', 'registrar_usuario', 'eliminar_usuario', 'guardar_proyecto'];

    if (!tablasCore.includes(instruccion)) {
        return res.status(400).json({ success: false, error: "Instrucción no válida." });
    }
    ...
    try {
        ...
        console.error(`Error en auditoría core.${instruccion}:`, error.message);
        res.status(500).json({ success: false, error: "Error al extraer datos de auditoría." });
    }
});
